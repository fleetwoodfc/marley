		...
		if (frm.doc.user_id && frm.doc.practitioner_type == "Internal") {
			frappe.call({
				...
				callback: function (data) {
					...
					if (
						!frm.doc.first_name ||
						frm.doc.first_name != data.message.first_name
					)
						frappe.model.set_value(
							frm.doctype,
							frm.docname,
					...
					if (
						!frm.doc.middle_name ||
						frm.doc.middle_name != data.message.middle_name
					)
						frappe.model.set_value(
							frm.doctype,
							frm.docname,
					...
					if (
						!frm.doc.last_name ||
						frm.doc.last_name != data.message.last_name
					)
						frappe.model.set_value(
							frm.doctype,
							frm.docname,
					...
					if (
						!frm.doc.mobile_phone ||
						frm.doc.mobile_phone != data.message.mobile_no
					)
						frappe.model.set_value(
							frm.doctype,
							frm.docname,
			...
			frappe.call({
				...
				callback: function (r) {
					if (r.message) {
						if (
							!frm.doc.supplier ||
							frm.doc.supplier != r.message["supplier"]
						) {
		...
		if (frm.doc.employee) {
			frappe.call({
				...
				callback: function (data) {
					...
					if (
						!frm.doc.designation ||
						frm.doc.designation != data.message.designation
					)
						frappe.model.set_value(
							frm.doctype,
							frm.docname,
					...
					if (
						!frm.doc.address ||
						frm.doc.address != data.message.current_address
					)
						frappe.model.set_value(
							frm.doctype,
							frm.docname,
							"address",
							data.message.current_address,
						);
				},
			});
		}
