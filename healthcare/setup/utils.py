...
def create_workspace_if_not_exists(title, items=None):
    ...
    items is a list of dicts: {"type": "doctype"|"page", "name": "<Name>"}
    """
    if not frappe:
        return
    if frappe.db.exists("Workspace", title):
        return
    ws = frappe.get_doc({"doctype": "Workspace", "title": title, "desktop_items": items or []})
    ws.insert(ignore_permissions=True)
    frappe.db.commit()

def create_doctype_link(doctype_name, module="Healthcare"):
    """
    Create a simple Link/DocType if it doesn't exist.
    This is a placeholder — in ERPNext you create DocTypes differently (via fixtures or doctypes).
    """
    if not frappe:
        return
    if frappe.db.exists("DocType", doctype_name):
        return
    # Minimal doc creation — in real ERPNext you'd define fields and metadata
    dt = frappe.get_doc({
        "doctype": "DocType",
        "module": module,
        "name": doctype_name,
